...
function cs_is_a_google_font(fontName){
...
function cs_prepare_fonts_import_code_snippet(){
	console.log('Running function cs_prepare_fonts_import_code_snippet to generate html code for font import:');
	...
	var first_part="";
	if ($("#_customize-input-SCSSvar_font-family-base").val().trim()!='' && cs_is_a_google_font($("#_customize-input-SCSSvar_font-family-base").val().split(',')[0].trim().replace(/"/g, "")) ) {  
	...
	var second_part="";
	if ($("#_customize-input-SCSSvar_headings-font-family").val().trim()!=''  && cs_is_a_google_font($("#_customize-input-SCSSvar_headings-font-family").val().split(',')[0].trim().replace(/"/g, "")) ) {
	...
	$(document).ready(function() {
		...
		//wp.codeEditor.initialize(jQuery('#_customize-input-customstrap_header_code'));
		...
		$("body").on("mousedown", "#customize-save-button-wrapper #save", function() {
			$("#_customize-input-customstrap_fonts_header_code").val(cs_prepare_fonts_import_code_snippet()).change();
		});			
		
		//LISTEN TO CUSTOMIZER CHANGES: if some variable is changed, we'll have to recompile
		wp.customize.bind( 'change', function ( setting ) {
			if (setting.id.includes("SCSSvar")  || setting.id.includes("body_font")   || setting.id.includes("headings_font")  || setting.id.includes("customstrap_fontawesome_disable") ) scss_recompile_is_necessary=true;
		});
		...
		*/
		...
		// FONT COMBINATIONS ////////////////////////////////////////////
		$("li#customize-control-SCSSvar_font-family-base").prepend(cs_font_combinations_select);
	
		//WHEN A FONT COMBINATION IS CHOSEN
		$("body").on("change", "select#_cs_font_combinations", function() {
			...
			//$('select#_cs_font_combinations option:first').attr('selected','selected');
		...
		$("body").on("change", "#_customize-input-SCSSvar_font-family-base", function() { //reset legacy font select
			$('select[data-customize-setting-link="customstrap_body_font"] option:first').attr('selected', 'selected').change();
		});
		...
		$("body").on("change", "#_customize-input-SCSSvar_headings-font-family", function() { //reset legacy font select
			$('select[data-customize-setting-link="customstrap_headings_font"] option:first').attr('selected', 'selected').change();
		});
		...
		$("body").on("click",".cs-toggle-csseditor-position",function(e){
			...
			$('#customize-control-custom_css').toggleClass('customstrap-maximize-editor');
		});
